        ...
        public async Task<ActionResult<Response<IReadOnlyList<PedidoResumenDto>>>> ObtenerPedidos([FromQuery] Guid clienteId)
        {
            ...
            try
            {
                ...
                if (!result.response)
                {
                    ...
                    return StatusCode(status, new Response<IReadOnlyList<PedidoResumenDto>>
                    {
                        Status = status,
                        Message = message,
                        Errors = new[] { message }
                ...
                return StatusCode(result.status, new Response<IReadOnlyList<PedidoResumenDto>>
                {
        ...
        public async Task<ActionResult<Response<DireccionClienteDto>>> ObtenerDireccion([FromQuery] Guid clienteId)
        {
            ...
            try
            {
                ...
                if (!result.response)
                {
                    ...
                    return StatusCode(status, new Response<DireccionClienteDto>
                    {
                        Status = status,
                        Message = message,
                        Errors = new[] { message }
                ...
                return StatusCode(result.status, new Response<DireccionClienteDto>
                {
                    Status = result.status,
                    Message = string.IsNullOrWhiteSpace(result.message)
